...
export class IoRedisAdapter implements CacheClient {
  ...
  private async init() {
    ...
    const versionFragment = infoString.split('\n').find((info) => info.includes(REDIS_VERSION_FRAGMENT_IDENTIFIER));
    this.redisVersion = versionFragment?.replace('\r', '').split(REDIS_VERSION_FRAGMENT_IDENTIFIER)[1] || '0';
  }
  ...
  public async keys(pattern: string): Promise<string[]> {
    ...
    while (cursor !== null) {
      const result = (await this.redisClient.scan(cursor, 'MATCH', pattern, 'COUNT', 1000)) as [string, string[]] | undefined;
