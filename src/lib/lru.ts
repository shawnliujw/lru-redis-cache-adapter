...
export class LRUCacheAdapter<T> implements CacheClient {
  ...
  public async set(cacheKey: string, value: any, ttl?: number): Promise<any> {
    ...
    return Promise.resolve(this.lruClient.set(cacheKey, value as T, ttl ? ttl * 1000 : undefined));
  }
...
export const useAdapter = <T>(client: LRUCache<string, T>, asFallback?: boolean): LRUCacheAdapter<T> => {
